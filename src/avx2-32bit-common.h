...
X86_SIMD_SORT_INLINE reg_t sort_ymm_32bit(reg_t ymm)
{
    ...
    ymm = cmp_merge<vtype>(
            ymm, vtype::template shuffle<SHUFFLE_MASK(2, 3, 0, 1)>(ymm), oxAA);
    ymm = cmp_merge<vtype>(
            ymm, vtype::permutexvar(vtype::seti(NETWORK_32BIT_1), ymm), oxCC);
    ymm = cmp_merge<vtype>(
            ymm, vtype::template shuffle<SHUFFLE_MASK(2, 3, 0, 1)>(ymm), oxAA);
    ymm = cmp_merge<vtype>(ymm, vtype::permutexvar(rev_index, ymm), oxF0);
    ymm = cmp_merge<vtype>(
            ymm, vtype::permutexvar(vtype::seti(NETWORK_32BIT_3), ymm), oxCC);
    ymm = cmp_merge<vtype>(
            ymm, vtype::template shuffle<SHUFFLE_MASK(2, 3, 0, 1)>(ymm), oxAA);
    return ymm;
}
...
struct ymm_vector<int32_t> {
    ...
    static inline int32_t partition_vec(type_t *arr,
                                        ...
                                        reg_t *biggest_vec)
    {
        return partition_vec_avx2<ymm_vector<type_t>>(arr,
                                                      left,
                                                      right,
                                                      curr_vec,
                                                      pivot_vec,
